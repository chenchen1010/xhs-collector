...
# ================= 配置信息 =================
API_URL = "http://localhost:8000/api/v1/collect/profile-info"
...
# 博主主页链接
USER_URL = "https://www.xiaohongshu.com/user/profile/682fe78f000000000a03f922?xsec_token=ABzVOsPMtTLS9q2Juy508dv-WoSQCptR1jGUa5VgnEIEY="

# 博主信息写入表格链接
TABLE_URL = "https://gcn6bvkburhk.feishu.cn/base/L5yObKSElaxLxUsmgM7cwR2unyd?table=tblBA6JLzTiH7s5r&view=vewYP6e30Y"
        ...
        try:
            ...
            if data.get("success"):
                ...
                if data.get("records"):
                    ...
                    print(f"\n--- 博主信息预览 ---")
                    print(f"账号名称: {fields.get('账号名称', '未知')}")
                    print(f"账号ID: {fields.get('账号ID', '未知')}")
                    print(f"粉丝数: {fields.get('粉丝数', 0)}")
                    print(f"获赞与收藏: {fields.get('获赞与收藏', 0)}")
                    print(f"账号简介: {fields.get('账号简介', '未知')[:50]}...")
                    print(f"------------------")
            else:
